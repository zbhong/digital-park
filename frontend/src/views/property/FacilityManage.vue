<template>
  <div>
    ...
    <el-table v-loading="loading" :data="dataList" border stripe>
      ...
      <el-table-column label="下次检查" prop="nextCheckDate" align="center" min-width="120" />
      <el-table-column label="状态" prop="status" align="center" min-width="80">
        <template #default="{ row }"><el-tag :type="row.status === 1 ? 'success' : 'danger'" size="small">{{ row.status === 1 ? '正常' : '维修中' }}</el-tag></template>
      </el-table-column>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12"><el-form-item label="设备类型" prop="type">
            <el-select v-model="form.type" placeholder="请选择类型" style="width: 100%">
              <el-option label="电梯" value="电梯" /><el-option label="空调" value="空调" /><el-option label="水泵" value="水泵" /><el-option label="配电" value="配电" /><el-option label="消防" value="消防" />
            </el-select>
          </el-form-item></el-col>
        </el-row>
        <el-row :gutter="20">
          ...
          <el-col :span="12"><el-form-item label="状态" prop="status">
            <el-select v-model="form.status" placeholder="请选择状态" style="width: 100%">
              <el-option label="正常" :value="1" /><el-option label="维修中" :value="0" />
            </el-select>
          </el-form-item></el-col>
        </el-row>
        <el-row :gutter="20">
          ...
          <el-col :span="12"><el-form-item label="下次检查" prop="nextCheckDate"><el-date-picker v-model="form.nextCheckDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" style="width: 100%" /></el-form-item></el-col>
        </el-row>
      </el-form>
      <template #footer>
...
import { getFacilityPage, getFacility, addFacility, updateFacility, deleteFacility } from '@/api/property'
...
function getList() {
  loading.value = true
  getFacilityPage(queryParams).then(res => { dataList.value = res.rows || res.data || []; total.value = res.total || 0 }).catch(() => {
    dataList.value = []
    total.value = 0
...
function resetForm() { form.value = { id: undefined, name: '', type: '', location: '', areaId: undefined, lastCheckDate: '', nextCheckDate: '', status: 1 }; if (formRef.value) formRef.value.resetFields() }
function handleAdd() { resetForm(); dialogTitle.value = '新增设备'; dialogVisible.value = true }
function handleEdit(row) { resetForm(); getFacility(row.id).then(res => { form.value = res.data || res; dialogTitle.value = '编辑设备'; dialogVisible.value = true }).catch(() => { form.value = { ...row }; dialogTitle.value = '编辑设备'; dialogVisible.value = true }) }
function submitForm() { formRef.value.validate(valid => { if (valid) { const api = form.value.id ? updateFacility : addFacility; api(form.value).then(() => { ElMessage.success(form.value.id ? '修改成功' : '新增成功'); dialogVisible.value = false; getList() }).catch(() => { ElMessage.error('操作失败') }) } }) }
function handleDelete(row) { ElMessageBox.confirm('是否确认删除该设备？', '系统提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => { deleteFacility(row.id).then(() => { ElMessage.success('删除成功'); getList() }).catch(() => { ElMessage.error('删除失败') }) }).catch(() => {}) }
